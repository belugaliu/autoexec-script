 ...
 * so the code like OracleDatabase.
 * @author liull
 */
public class DmDatabase extends Database<DmConnection> {
    ...
    public final void ensureSupported() {
        ...
        ensureDatabaseNotOlderThanOtherwiseRecommendUpgradeToFlywayEdition("7.6.0", org.flywaydb.core.internal.license.Edition.ENTERPRISE);
        // 暂时不动
...
//        return getMainConnection().getJdbcTemplate().queryForString("SELECT GLOBAL_NAME FROM GLOBAL_NAME");
    ...
    private Set<String> getAvailableOptions() throws SQLException {
        return new HashSet<>(getMainConnection().getJdbcTemplate()
                                     .queryForStringList("SELECT PARA_NAME FROM V$OPTION WHERE PARA_VALUE = '1'"));
    }

    private Set<String> getAvailableParamter() throws SQLException {
        return new HashSet<>(getMainConnection().getJdbcTemplate()
                .queryForStringList("select name from v$parameter value = '1'"));
    }

    /**
     * Checks whether Flashback paramter is available or not.
     *
    ...
    boolean isFlashbackDataArchiveAvailable() throws SQLException {
        return getAvailableParamter().contains("ENABLE_FLASHBACK");
    }
